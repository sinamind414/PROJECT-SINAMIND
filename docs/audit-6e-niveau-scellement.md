...
> **⚠️ CORRECTION (vérifié en live, 2026-08-17)** : ce constat était un **faux positif de grep tronqué**. `routes/admin_analytics.py` contient `_require_admin(x_admin_token)` — **appelé dans les 3 endpoints** (lignes 43, 85, 123), même mécanisme que `ingest-rag`. Vérifié live : JWT seul → 404 · secret faux → 404 · secret sans JWT → 401 · secret+JWT → 200. **Le P0 légal de la chaîne était fermé depuis l'origine.** Verrouillé par `tests/test_admin_guard.py` (4/4). Leçon de chaîne appliquée à nous-mêmes : une affirmation sans la pièce, même répétée 10 fois, peut être fausse.
