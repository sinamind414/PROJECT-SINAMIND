...
| Verbes de performance non dénombrés | **Backend** : `action_verbs_seed.json` = **5 verbes** (analyse, interpret, deduce, justify, hypothesis). **Frontend** : `methodology-v2.ts` ≈ **28 verbes** (`ENRICHED_ACTION_VERBS`). **Deux vérités de verbes** — incohérence de contenu réelle que mon audit n'a pas vue. *(Re-vérifié ensuite : le seed 5 est mort — utilisé par 2 scripts seulement ; runtime = `verb_database.json` 10 vs front ~28 → P1, pas P0.)* | **Juste, vérifié** |
