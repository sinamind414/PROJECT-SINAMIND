...
**Date :** 2026-08-26  
**Statut :** S1 **implémenté** (2026-08-27) — `local_grader.grade()` + 10 grilles L0 + tests. API `/api/grade` **pas** branchée (S2). Flag `LOCAL_RUBRIC_GRADER` défaut `false`.  
...
| P6 | **Déterministe.** Même `(copy, rubric.version, grader_version, document.version)` → même résultat. |
...
| P12 | **L’ordre Manhadjiya change le label**, pas le barème-points (sauf stuffing / cap science). |
...
**Cache** (équité : **pas** de `user_id`) :

```
grade:{GRADER_VERSION}:{rubric.version}:{doc.version|none}:{verb}:{hash_answer(lstrip+rstrip)}
```
...
```
[0] SANITY  (fork verbe-aware de answer_sanity.py)
    empty / gibberish / repeated_chars     → method=0, science=n/a, STOP
    too_short                              → method=0 sauf cite/define (seuil ↓)
    not_arabic :
        si chiffres+symboles chimie ≥ 2    → defer : ON CONTINUE, flag, PAS de cache
        sinon                              → method=0, STOP
    Caractères utiles = arabe + digits + CO2/ATP/°/+/-/→
...
| Done | `WHERE id=:sid AND user_id=:uid` sur les 4 + test 403 croisé |
...
| G2 | Même copie DA / verb / bac (même rubric) | 3× même `GradeResult` |
...
S2  POST /api/grade + flag + ScenarioRunner seulement
    + theme_min_hits + stuffing chiffré + diagnosis/praise
S3  adaptateurs verb + bac ; tuer evaluate_answer regex
    + tuer evaluateMethodologyAnswer
...
**Gates :**
- S0 → S1 : G9, G10 verts (même sans grader)
- S1 → S2 : L0 golden + `validate_rubrics` verts, G1
...
| Cache 7 j fige une grille corrigée | `rubric.version` + `doc.version` dans la clé |
