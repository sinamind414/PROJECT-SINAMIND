...
| CI grader-tests | **PRÊT** | Job dédié (snippet `docs/ci-grader-tests.job.yml`, déploiement pending permission workflows) : `pytest tests/test_local_grader.py tests/test_grade_s*.py …tests/test_grade_bac2023_ml901.py --noconftest` + `validate_rubrics.py`. Fix audit F6 (suite 214 ✓ / 0 ✗, tests S3/S9/S25 réalignés sur le mur HON-2). |
...
| L0+ علّل/قارن greffe | **OUI** | `greffe-ltc-justify` + `greffe-ltc-compare`. Mêmes keypoints 10/5/2,5/4,8. **0 Y**. `1.1.7`. |
