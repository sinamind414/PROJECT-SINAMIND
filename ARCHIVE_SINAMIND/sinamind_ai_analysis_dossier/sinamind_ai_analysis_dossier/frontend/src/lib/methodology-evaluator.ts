import { getCorrectionLexiconContext } from "@/lib/correction-lexicon"
...
type EvaluatorLexiconContext = ReturnType<typeof getCorrectionLexiconContext>
...
function expandWithContext(markers: string[], context?: EvaluatorLexiconContext) {
  if (!context) return unique(markers)
  const expanded = [...markers]
  markers.forEach((marker) => {
    const normalizedMarker = normalize(marker)
    const synonymGroup = context.acceptedSynonyms[normalizedMarker]
    if (synonymGroup?.length) expanded.push(...synonymGroup)
  })
  return unique(expanded)
}

function foundMarkers(answer: string, markers: string[], context?: EvaluatorLexiconContext) {
  const normalizedAnswer = normalize(answer)
  return expandWithContext(markers, context).filter((marker) => normalizedAnswer.includes(normalize(marker)))
}

function getEnrichedMarkers(base: string[], context?: EvaluatorLexiconContext, extra: string[] = []) {
  return unique([...base, ...extra, ...(context ? expandWithContext([], context) : [])])
}

function getKnowledgeVocabulary(context?: EvaluatorLexiconContext) {
  const base = ["نعلم", "بروتين", "انزيم", "مورثه", "مورثة", "adn", "arn", "ريبوزوم", "خلية", "الخلايا"]
  return unique([...base, ...(context?.scientificVocabulary || []), ...(context?.acceptedConcepts || [])])
}
...
function buildBaseEvaluation(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const forbiddenMarkersFound = foundMarkers(answer, verb.forbiddenMarkers, context)
  const missingMarkers = verb.requiredMarkers.filter((marker) => !foundMarkers(answer, [marker], context).length)
...
function evaluateAnalyse(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  const success = evaluation.success
  const errors = evaluation.errors
  const criteria = evaluation.criteria

  const hasDocument = includesAny(normalizedAnswer, unique([...DOCUMENT_MARKERS, ...(context?.observationTerms || [])]))
  const hasVariation = includesAny(normalizedAnswer, unique([...VARIATION_MARKERS, ...(context?.observationTerms || []), ...(context?.relationTerms || [])]))
  const hasNumericValue = hasNumber(answer)
  const hasForbidden = evaluation.forbiddenMarkersFound.length > 0
  const hasVariables = includesAny(normalizedAnswer, unique(["بدلاله", "بدلالة", "الزمن", "كميه", "كمية", "تركيز", "نشاط", "نسبه", "نسبة", ...(context?.variableTerms || [])]))
...
function evaluateInterpret(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  const success = evaluation.success
  const errors = evaluation.errors
  ...
  const hasObservationLink = includesAny(normalizedAnswer, unique(["نلاحظ", "النتيجه", "النتيجة", "هذا", "ذلك", "انخفاض", "ارتفاع", ...(context?.observationTerms || [])]))
  const hasCausal = includesAny(normalizedAnswer, unique([...CAUSAL_MARKERS, ...(context?.causalTerms || [])]))
  const hasKnowledge = includesAny(normalizedAnswer, getKnowledgeVocabulary(context))
...
function evaluateDeduce(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  ...
  const hasDeduction = includesAny(normalizedAnswer, unique([...DEDUCTION_MARKERS, ...(context?.deductionTerms || [])]))
...
function evaluateHypothesis(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  const success = evaluation.success
  const errors = evaluation.errors
  ...
  const hasHypothesisMarker = includesAny(normalizedAnswer, unique(["نفترض", "يعود سبب", "نقترح", "الفرضيه", "الفرضية", ...(context?.hypothesisTerms || [])]))
  const hasCausal = includesAny(normalizedAnswer, unique(["سبب", "نتيجه", "نتيجة", "خلل", "تؤدي", "يؤدي", "بسبب", ...(context?.causalTerms || [])]))
  const hasForbidden = evaluation.forbiddenMarkersFound.length > 0
  const testableProxy = includesAny(normalizedAnswer, unique(["مورثه", "مورثة", "بروتين", "انزيم", "تركيز", "نشاط", "طفره", "طفرة", "دواء", "تجربه", "تجربة", ...(context?.acceptedConcepts || []), ...(context?.scientificVocabulary || [])]))
...
function evaluateScientificText(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  const success = evaluation.success
  const errors = evaluation.errors
...
function evaluateGeneric(verb: ActionVerbRule, answer: string, context?: EvaluatorLexiconContext): MethodologyEvaluation {
  const evaluation = buildBaseEvaluation(verb, answer, context)
  const normalizedAnswer = normalize(answer)
  const success = evaluation.success
  const errors = evaluation.errors
  ...
  const requiredMarkers = expandWithContext(verb.requiredMarkers, context)
  const hasRequired = requiredMarkers.length === 0 || requiredMarkers.some((marker) => normalizedAnswer.includes(normalize(marker)))
  const noForbidden = evaluation.forbiddenMarkersFound.length === 0
  const longEnough = answer.trim().length >= 30
  const categorySpecific =
    verb.slug === "compare" ? includesAny(normalizedAnswer, unique([...COMPARISON_MARKERS, ...(context?.comparisonTerms || [])])) :
    verb.slug === "relationship" ? includesAny(normalizedAnswer, unique([...RELATION_MARKERS, ...(context?.relationTerms || [])])) :
    true
...
export function evaluateMethodologyAnswer(input: EvaluateMethodologyInput): MethodologyEvaluation {
  ...
  const lexiconContext = getCorrectionLexiconContext({
  ...
  if (verb.slug === "analyse") return evaluateAnalyse(verb, answer, lexiconContext)
  if (verb.slug === "interpret") return evaluateInterpret(verb, answer, lexiconContext)
  if (verb.slug === "deduce") return evaluateDeduce(verb, answer, lexiconContext)
  if (verb.slug === "hypothesis") return evaluateHypothesis(verb, answer, lexiconContext)
  if (verb.slug === "scientific-text") return evaluateScientificText(verb, answer, lexiconContext)

  return evaluateGeneric(verb, answer, lexiconContext)
}
