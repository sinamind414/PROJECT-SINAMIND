...
type QuestionId = MethodologyQuestion["id"]
...
function buildDiagnosticResult(answers: Record<string, string>): DiagnosticResult {
  const evaluations = QUESTIONS.map((question) => ({
    ...
    evaluation: evaluateMethodologyAnswer({
      ...
      unitKey: diagnosticScenario.unitKey,
    }),
  }))
  ...
  else if (weak.some((item) => item.question.verbSlug === "analyse")) profileAr = "مشكلتك الأساسية أنك لا تستغل الوثائق كما يجب، وهذا سيكلفك نقاطا مباشرة."
  else if (weak.some((item) => item.question.verbSlug === "hypothesis")) profileAr = "مشكلتك الكبرى في المسعى العلمي: الفرضيات غير مرتبطة بما يكفي بالوثائق."
  else if (weak.some((item) => item.question.verbSlug === "scientific-text")) profileAr = "معارفك قد تكون موجودة، لكنك لا تركبها في نص علمي يستغل الوثائق."
