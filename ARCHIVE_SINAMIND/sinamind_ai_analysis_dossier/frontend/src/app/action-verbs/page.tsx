            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {actionVerbs.map((verb) => (
                <Link
                  ...
                  className="rounded-2xl p-5 border border-white/[0.06] bg-[#2A2540] hover:bg-white/[0.06] transition group"
                >
                  ...
                  <div className="h-2 rounded-full bg-white/[0.06] overflow-hidden mb-3">
                    <div className="h-full rounded-full bg-violet-400" style={{ width: `${verb.level}%` }} />
                  </div>
                  <p className="text-gray-500 text-xs">آخر خطأ: {verb.lastError}</p>
                  <p className="text-violet-300 text-sm font-bold mt-4 group-hover:underline">ابدأ التدريب ←</p>
                </Link>
              ))}
            </div>
          </div>
