/* ============================================
   WILAYAS.JS - Liste des 58 wilayas d'Algérie
   ============================================ */
...
function populateWilayas(selectElement) {
  ...
  wilayas.forEach(wilaya => {
    ...
    option.textContent = `${wilaya.code} - ${wilaya.name_ar} (${wilaya.name_fr})`;
    selectElement.appendChild(option);
  });
}

// Récupérer le nom d'une wilaya par code
function getWilayaName(code) {
  const wilaya = wilayas.find(w => w.code === code);
  return wilaya ? `${wilaya.name_ar} (${wilaya.name_fr})` : '';
}

if (typeof window !== 'undefined') {
  window.wilayas = wilayas;
  window.populateWilayas = populateWilayas;
  window.getWilayaName = getWilayaName;
}
