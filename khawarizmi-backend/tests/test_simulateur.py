...
def main():
    ...
    for i, scenario in enumerate(SCENARIOS, 1):
        ...
        if pre_analyse:
            ...
            safe_print(
                f"   → Type détecté : {pre_analyse['type_erreur']} "
...
def _appel_ia_optionnel(prompt: str, student_input: str):
    ...
    if not api_key or len(api_key) < 20 or api_key in ("sk-...", "METTRE_TA_CLE_ICI"):
        safe_print("\n[INFO] Appel IA ignoré — Définis OPENAI_API_KEY dans .env pour tester l'appel réel.")
        return
    ...
    try:
        ...
        response = client.chat.completions.create(
            model="gpt-4o-mini",
            messages=[{"role": "system", "content": prompt}, {"role": "user", "content": student_input}],
            temperature=0.2,
            max_tokens=500,
        ...
        safe_print("-" * 40)
        safe_print(
            f"[MÉTRIQUES] tokens={response.usage.total_tokens} | coût≈${response.usage.total_tokens * 0.00000015:.6f}"
        )
