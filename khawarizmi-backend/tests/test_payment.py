...
def test_webhook_invalid_signature():
    response = client.post(
        "/api/payment/webhook/chargily", content='{"event":"checkout.paid"}', headers={"signature": "bad_sig"}
    )
    assert response.status_code == 403
    data = response.json()
    assert data["erreur"] == "Invalid signature"
...
async def test_webhook_paid_success():
    body_dict = {
        "event": "checkout.paid",
        "data": {"id": "chk_999", "status": "paid", "amount": 990.00, "metadata": {"user_id": "42"}},
    }
    ...
    with patch("routes.payment.get_state") as mock_get_state:
        ...
        response = client.post(
            "/api/payment/webhook/chargily", content=json.dumps(body_dict), headers={"signature": signature}
        )
...
async def test_webhook_idempotency():
    body_dict = {"event": "checkout.paid", "data": {"id": "chk_already_paid", "status": "paid", "amount": 990.00}}
    body_bytes = json.dumps(body_dict).encode("utf-8")
    signature = hmac.new(CHARGILY_SECRET_TEST_KEY.encode("utf-8"), body_bytes, hashlib.sha256).hexdigest()
