...
def tutor():
    ...
    data_dir = os.getenv("DATA_DIR", str(Path(__file__).parent.parent / "LIVRES SCOLAIRES"))
...
# ═══ CHEMIN DONNÉES ═══════════════════════════════════════
DATA_DIR = os.environ.get("DATA_DIR", str(Path(__file__).parent.parent / "LIVRES SCOLAIRES"))
...
async def tester_scenario(tutor: KhawarizmiTutor, scenario: dict) -> dict:
    ...
    pre_analyse = tutor.pre_analyser_sans_ia(scenario["sujet_id"], scenario["question_id"], scenario["input"])
    if pre_analyse:
    ...
    try:
        response = await client.chat.completions.create(
            model=MODEL,
            messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": scenario["input"]}],
            temperature=0.3,
            max_tokens=500,
            timeout=30.0,
        )
        ...
        try:
            ...
            print(f"   [JSONDecodeError] raw content: {content!r}")
        ...
        if type_attendu:
            ...
            print(f"   🎯 Type erreur    : {type_recu} ({'✅ correct' if match else f'❌ attendu {type_attendu}'})")
...
async def main():
    ...
    temps_moy = sum(r.get("temps", 0) for r in resultats if r.get("succes")) / max(succes, 1)
