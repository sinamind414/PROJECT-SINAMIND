...
class TestRegister:
    async def test_register_success(self, client: AsyncClient):
        ...
        assert response.status_code == 200  # déterminé (audit technique 2026-08-18)
    ...
    async def test_register_duplicate_email(self, client: AsyncClient):
        ...
        assert response.status_code == 400  # déterminé : email déjà inséré
    ...
    async def test_register_invalid_email(self, client: AsyncClient):
        ...
        assert response.status_code == 422  # déterminé : email invalide
...
class TestToken:
    ...
    async def test_missing_token_rejected(self, client: AsyncClient):
        response = await client.get("/api/auth/me")
        assert response.status_code == 401  # déterminé : token absent -> 401
