...
- 2e envoi identique → hit cache, 0 appel LLM, source="cached_evaluation"
- Normalisation d'espaces sûre : les highlights sont reprojetés (+delta),
  le span pointe toujours le bon mot
...
- Bump de version de prompt → invalidation
- Single-flight : 10 corrections concurrentes identiques → 1 seul appel LLM
- Aucun llm_raw / llm_raw_hash dans le payload caché
"""
...
from services import grading_cache
from services.grading_cache import (
    CACHE_TTL_SECONDS,
...
def _llm_result(score: int = 5, highlights=None) -> dict:
    return {
        "source": "llm",
        "score": score,
        "score_max": SMAX,
...
class TestKeyNormalize:
    def test_crlf_converted_to_lf(self):
        assert key_normalize("a\r\nb") == ("a\nb", 0)

    def test_leading_whitespace_gives_delta(self):
...
class TestBuildCorrectionKey:
    def test_same_content_different_whitespace_same_key(self):
...
class TestCorrectionCache:
    async def test_second_identical_answer_hits_cache(self, fake_redis):
        """Acceptation C2 : même réponse envoyée 2 fois → 2e correction
        source=cached_evaluation, 0 appel LLM."""
        ...
        assert r2["source"] == "cached_evaluation"
    ...
    async def test_trailing_newline_hits_and_offsets_shift(self, fake_redis):
        """Acceptation C2 : \n\n + réponse → hit, highlights reprojetés +2,
        le span pointe toujours le bon mot."""
    ...
    async def test_sanity_rejection_is_not_cached(self, fake_redis):
        """Les rejets sanity (0 token, ~0 ms) ne sont pas cachés : le cache
        serait plus lent que la correction elle-même."""
        ...
        await _call(evaluate_fn, "ZZZZ")
        await _call(evaluate_fn, "ZZZZ")
        assert evaluate_fn.await_count == 2

    async def test_prompt_version_bump_invalidates(self, fake_redis, monkeypatch):
        """Le bump de CORRECTION_PROMPT_VERSION invalide sélectivement."""
        evaluate_fn = _make_evaluate_fn(return_result=_llm_result(score=5))
        await _call(evaluate_fn, ANS)
        monkeypatch.setattr(grading_cache, "CORRECTION_PROMPT_VERSION", "p2")
    ...
    async def test_concurrent_identical_answers_single_llm_call(self, fake_redis):
        ...
        assert sum(1 for r in results if r["source"] == "cached_evaluation") == 9
        assert sum(1 for r in results if r["source"] == "llm") == 1

    async def test_no_llm_raw_in_cached_payload(self, fake_redis):
        """Aucun llm_raw / llm_raw_hash dans le payload caché (contrat public)."""
        evaluate_fn = _make_evaluate_fn(return_result=_llm_result(score=5))
        await _call(evaluate_fn, ANS)
    ...
    async def test_miss_returns_highlights_in_raw_space(self, fake_redis):
        ...
        convention qu'au hit — sinon le frontend désaiglerait)."""
        evaluate_fn = _make_evaluate_fn(return_result=_llm_result(score=5))
        r = await _call(evaluate_fn, "   " + ANS)
        assert r["source"] == "llm"
    ...
    async def test_cache_disabled_without_redis(self):
        """Sans Redis (CI), le wrapper se comporte comme un simple appel."""
        evaluate_fn = _make_evaluate_fn(return_result=_llm_result(score=5))
        r1 = await _call(evaluate_fn, ANS)
        r2 = await _call(evaluate_fn, ANS)
        assert evaluate_fn.await_count == 2
    ...
    async def test_ttl_is_14_days(self):
        assert CACHE_TTL_SECONDS == 14 * 24 * 3600


# ── Tests unitaires : is_cacheable / project ─────────────────────────

class TestIsCacheable:
    def test_llm_ok_cacheable(self):
        assert is_cacheable(_llm_result(score=5)) is True

    def test_llm_recovered_recovered_cacheable(self):
        r = _llm_result(score=5)
        r["source"] = "llm_recovered"
        r["parse_status"] = "recovered"
