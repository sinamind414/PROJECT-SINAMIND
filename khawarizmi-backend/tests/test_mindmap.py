...
class TestMindMapAuth:
    async def test_generate_requires_auth(self, client: AsyncClient):
        response = await client.post("/api/mindmap/generate", json=VALID_REQUEST)
        assert response.status_code in [401, 503]


class TestMindMapMaitrise:
    async def test_update_maitrise_invalid_value(self, client: AsyncClient, auth_headers: dict):
        response = await client.patch("/api/mindmap/fake-node/maitrise", headers=auth_headers, json={"maitrise": 5})
...
class TestMindMapDynamicGeneration:
    async def test_generate_no_context(self, client: AsyncClient, auth_headers: dict):
        ...
        with (
            patch("services.embedder.embedder.encode") as mock_encode,
            ...
            response = await client.post("/api/mindmap/generate", headers=auth_headers, json=VALID_REQUEST)
    ...
    async def test_generate_success(self, client: AsyncClient, auth_headers: dict):
        ...
        mock_openai.chat.completions.create = AsyncMock(return_value=MagicMock(choices=[mock_choice]))
