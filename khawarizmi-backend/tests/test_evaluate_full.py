...
class MockScheduler:
    def calculer_prochain_intervalle(self, card, score_percent):
        ...
        return {
            "card": mock_card,
            "prochaine_revision": datetime.now(UTC) + timedelta(days=2),
            "interval_jours": 2,
            "difficulty": 0,
            "stability": 0,
        }
...
def run_tests():
    ...
    with patch("services.llm.call_gpt4o_evaluator") as mock_llm:
        ...
        res2 = client.post(
            "/api/evaluate",
            json={"question_id": "q_test", "reponse_eleve": "les anticorps détruisent les antigènes", "tentative": 1},
        )
...
if __name__ == "__main__":
    ...
    class TestL2Evaluator:
        ...
        def test_embedding_quality_vs_sentence_transformers(self):
            ...
            correlations = [float(np.dot(st_embs[i], onnx_embs[i])) for i in range(len(test_phrases))]
            assert all(c > 0.97 for c in correlations), f"Dégradation trop importante : {correlations}"
    ...
    class TestFSRSConfig:
        def test_phase_transitions(self):
            ...
            with patch("services.fsrs_config.date") as mock_date:
                ...
                mock_date.side_effect = lambda *args, **kwargs: date(*args, **kwargs)
                scheduler = get_fsrs_scheduler()
                assert abs(scheduler.desired_retention - 0.90) < 1e-6
