...
class DualCodingService:
    ...
    async def evaluer_schema_photo(
        ...
        # ── Appel Vision IA ────────────────────────────────────
        logger.info(
            f"Vision IA : schema={schema_id} "
            f"taille={len(image_base64)//1024}Ko"
        )
    ...
    def _build_vision_prompt(self, schema_ref: Dict) -> str:
        ...
        points = '\n'.join(
            f'→ {p}' for p in schema_ref['points_critiques']
        )
