...
def get_fsrs_scheduler(user_fsrs_config: dict = None) -> Scheduler:
    ...
    if user_fsrs_config and "desired_retention" in user_fsrs_config:
        ...
        logger.info(f"FSRS calibration dynamique - Phase: {phase}, Retention cible: {desired_retention:.2f}, Jours restants BAC: {days_to_bac}")
    ...
    try:
        ...
        logger.warning(f"Paramètres FSRS non supportés par le constructeur Scheduler ({e}), fallback avec desired_retention={desired_retention}")
