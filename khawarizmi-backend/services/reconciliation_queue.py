    ...
    async with state.db_session() as db:
        try:
            ...
            res_config = await db.execute(
                text("SELECT fsrs_config FROM users WHERE id = :uid"), {"uid": int(student_id)}
            )
            ...
            await db.commit()
            logger.info(
                f"FSRS mis à jour avec succès après réévaluation pour user={student_id}, question={question_id}"
            )
        ...
        for review in batch:
            try:
                ...
                l1_result = await asyncio.wait_for(
                    call_gpt4o_evaluator(client=state.openai, question=question, reponse=review.answer, tentative=1),
                    timeout=15.0,
                )
                ...
                if abs(l1_score_norm - review.l2_score) > 0.20:
                    logger.warning(
                        f"Écart de score détecté (L1: {l1_score_norm:.2f}, L2: {review.l2_score:.2f}). Correction FSRS requise."
                    )
                    ...
                    async with state.db_session() as db:
                        ...
                        await db.commit()
                        logger.info(
                            f"Score L2 confirmé (L1: {l1_score_norm:.2f}, L2: {review.l2_score:.2f}) pour user={review.student_id}, question={review.question_id}"
                        )
            except Exception as e:
