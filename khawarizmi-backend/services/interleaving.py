...
class InterleavingSession:
    ...
    async def generer_session(
        ...
        chapitres_selec = await self._selectionner_chapitres(
            user_id, db, chapitres_matiere, mode
        )
        ...
        for chapitre_id in chapitres_selec:
            questions_ch = await self._get_questions_chapitre(
                user_id, db, chapitre_id, questions_par_ch
            )
        ...
        logger.info(
            f"Session interleaving générée : "
            f"user={user_id} matiere={matiere} "
            f"nb_questions={len(toutes_questions)}"
        )
        ...
        return {
            ...
            'message':          self._get_message_motivation(
                                    len(toutes_questions),
                                    list(stats_chapitres.keys())
                                ),
        }

    # ═══ SÉLECTION DES CHAPITRES ════════════════════════════

    async def _selectionner_chapitres(
        self,
        user_id:    int,
        db,
        chapitres:  List[str],
    ...
    async def _get_questions_chapitre(
        self,
        user_id:     int,
        db,
        chapitre_id: str,
        ...
        try:
            ...
            if not rows:
                logger.info(
                    f"Aucune question due pour {chapitre_id} "
                    f"(user={user_id}) → fallback questions nouvelles"
                )
                return []
    ...
    def _session_vide(self, matiere: str, user_id: int) -> Dict[str, Any]:
        ...
        logger.info(
            f"Session vide pour user={user_id} matiere={matiere} "
            f"→ Toutes les révisions sont à jour"
        )
        return {
            'mode':         'INTERLEAVING',
            'matiere':      matiere,
    ...
    def calculer_score_session(resultats: List[Dict]) -> Dict[str, Any]:
        ...
        score_global = round(
            sum(s for r in resultats for s in [r.get('score', 0.0)])
            / len(resultats), 1
        )
