...
JSON_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "data", "lexique_svt_terminale_complet.json")
...
def create_table():
    with ENGINE.connect() as conn:
        res = conn.execute(
            text("SELECT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'lexique_termes')")
        )
        if res.scalar():
