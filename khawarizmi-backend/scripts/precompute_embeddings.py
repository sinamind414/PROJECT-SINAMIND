...
async def precompute_and_store():
    ...
    async with engine.connect() as conn:
        try:
            ...
            if count is not None and count >= expected_minimum:
                print(
                    f"Embeddings are already precomputed ({count} rows found in DB, expected minimum: {expected_minimum}). Skipping computation."
                )
    ...
    async with engine.begin() as conn:
        ...
        for q_id, q_data in questions_db.items():
            ...
            for i, (text_val, emb) in enumerate(zip(reponses_ref, embeddings, strict=False)):
                await conn.execute(
                    text("""
                        ...
                        DO UPDATE SET
                            ...
                            updated_at = NOW()
                    """),
                    {"qid": q_id, "vidx": i, "text": text_val, "emb": str(emb.tolist())},
                )
            annales_count += 1
        ...
        if os.path.exists(exercises_path):
            ...
            for i in range(0, len(exercice_texts), batch_size):
                ...
                for eid, text_val, emb in zip(batch_ids, batch_texts, batch_embeddings, strict=False):
                    await conn.execute(
                        text("""
                            INSERT INTO reference_embeddings
                                (question_id, variant_index, reference_text, embedding, source)
                            ...
                            DO UPDATE SET
                                ...
                                updated_at = NOW()
                        """),
                        {"qid": eid, "text": text_val, "emb": str(emb.tolist())},
                    )
