...
function resetSocraticChat() {
    ...
    const initText = t('feynman_chat_init', "Marhaban ! Explique-moi la cinétique chimique...");
    history.innerHTML = `
...
function initWaitlistModal() {
    ...
    if (form) {
        form.addEventListener('submit', async (e) => {
            ...
            const WEBHOOK_URL = 'https://khawarizmi-ia-production-7837.up.railway.app/api/waitlist';
...
function initPhase2() {
    ...
    submitBtn.addEventListener('click', (e) => {
        ...
        // Validation (externalisable vers API)
        const checks = {
            hasFormula:    /u\s*=\s*r\s*[×x*·.\s]?\s*i/i.test(answer)
                        || answer.replace(/\s/g,'').includes('u=ri')
                        || answer.replace(/\s/g,'').includes('u=r×i'),
            hasTension:    /tension|volt|توتر|فولط|فولت/.test(answer),
            hasResistance: /résist|ohm|مقاوم/.test(answer),
            hasCourant:    /courant|intensité|ampère|تيار|شدة|أمبير/.test(answer)
        };

        let score = 0;
        if (checks.hasFormula)    score += 2;
        if (checks.hasTension)    score += 1;
        if (checks.hasResistance) score += 1;
        if (checks.hasCourant)    score += 1;
    ...
    submitBtn.addEventListener('click', (e) => {
        ...
        // Étape finale : Évaluation complète
            ...
            const conceptPatterns = {
                vitesse:       /vitesse|speed|rate|سرعة/i,
                reaction:      /réaction|reaction|تفاعل/i,
                temperature:   /température|temperature|حرارة|درجة/i,
                concentration: /concentration|تركيز/i,
                exemple:       /exemple|example|cuisson|rouille|rust|cook|صدأ|طبخ|citron|lemon/i,
                temps:         /temps|durée|time|مدة|زمن/i
            };

            const labelsFr = {
                vitesse: 'Vitesse', reaction: 'Réaction', temperature: 'Température',
                concentration: 'Concentration', exemple: 'Exemple pratique', temps: 'Temps/Durée'
            };
            const labelsAr = {
                vitesse: 'السرعة', reaction: 'التفاعل', temperature: 'درجة الحرارة',
                concentration: 'التركيز', exemple: 'مثال واقعي', temps: 'الزمن/المدة'
            };
