...
// Atelier satellite générique (صف · عرّف · أثبت · اقترح فرضية · ناقش) :
// même machine que les 7 jours (روتين → بطاقة → ورشة أ/ب/ج),
// métier détecté par liste fermée d'obligatoires/interdits du JSON.
// 0 appel API, 0 LLM, 0 note /10.
export function AtelierSatellite({ data, onReplay, variant = "satellite" }: Props) {
  ...
  const live = useMemo(() => detectSatellite(text, data), [text, data])
  ...
  const submit = () => {
    const d = detectSatellite(text, data)
    setSubmitted({ text, crimes: d.crimes, missing: d.missing })
    setPhase("C")
  }

  const finalDet = useMemo(
    () => (submitted ? detectSatellite(submitted.text, data) : null),
    [submitted, data]
  )
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Phase B — obligatoires/interdits du métier, détection locale */}
      {phase === "B" && (
        <section className="rounded-3xl border border-white/15 bg-slate-panel/60 p-5 space-y-4">
          <div className="flex items-center justify-between">
            <h2 className="font-black text-white" dir="rtl">
              المرحلة ب — اكتب
            </h2>
      ...
      {phase === "C" && finalDet && (
        <section className="space-y-4">
          ...
          {/* Un seul lien, en bas du miroir, après envoi : قمر → قمر (boucle) */}
          {data.lien_suivant && (
