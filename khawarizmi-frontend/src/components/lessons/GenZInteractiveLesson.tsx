...
function QCMItem({ item, onCorrect }: { item: { q: string; o: string[]; c: number }; onCorrect: () => void }) {
  ...
  return (
    <div className="mb-4 last:mb-0 bg-black/40 p-4 rounded-2xl">
      <div className="text-white font-bold text-sm mb-2">{item.q}</div>
      {item.o.map((o, j) => (
        <button
          key={j}
          disabled={sel !== null}
...
export default function GenZInteractiveLesson({ lessonTitleAr, slug, phases, onComplete }: Props) {
  ...
  const renderFrame = (phase: Phase, index: number) => {
    ...
    return (
      <div key={phase.id} className="flex-shrink-0 w-[90%] md:w-[78%] snap-start rounded-3xl border border-white/10 bg-white/[0.015] overflow-hidden mr-3 last:mr-0">
        <div className="flex items-center gap-4 px-6 py-5 bg-gradient-to-r from-white/5 border-b border-white/10">
          <div className="h-9 w-9 flex items-center justify-center rounded-full bg-mint text-black text-[22px] font-black">{num}</div>
          <div>
            <div className="font-black text-[21px] tracking-[-0.3px] text-white">{phase.titleAr}</div>
            <div className="text-[10px] text-white/40">إطار {num} / {totalFrames}</div>
          </div>
        </div>

        <div className="px-6 pt-6 pb-7">
          <div className="text-[15px] leading-relaxed text-white/90 mb-6">{phase.content}</div>

          {isACh && num === 2 && (
            <div className="mb-6 p-5 rounded-2xl bg-[#052e1e] border border-emerald-700/60">
              <div className="text-emerald-400 font-bold text-sm mb-2">محاكي النقل المشبكي</div>
              <div className="grid grid-cols-3 gap-2">
                {[{ l: "طبيعي", t: "normal" }, { l: "Curare", t: "curare" }, { l: "مثبط", t: "inhib" }].map((s) => (
                  <button key={s.t} onClick={() => runSim(s.t, phase.id)} className="text-xs py-2.5 rounded-2xl bg-emerald-900/40 border border-emerald-500/30 font-bold">
                    {s.l}
                  </button>
                ))}
              </div>
              {Object.keys(simStates)
                ...
                .map((k) => (
                  <div key={k} className="mt-3 text-xs p-3 bg-black/60 rounded-xl border-l-4 border-mint">
                    {simStates[k]}
                  </div>
                ))}
            </div>
          ...
          {phase.practice && (
            <div className="rounded-3xl border border-mint/30 bg-mint/5 p-5">
              <div className="text-mint font-black text-xs mb-2">دورك الحين يا خويا</div>
              <textarea
                value={ans}
                onChange={(e) => updateAnswer(phase.id, e.target.value)}
                placeholder="اكتب إجابتك..."
                className="w-full min-h-[110px] bg-black/30 border border-white/10 rounded-2xl p-4 text-white text-sm"
                dir="rtl"
              />
              <div className="flex gap-2 mt-3">
                <button onClick={() => submitPractice(phase.id)} disabled={!ans.trim()} className="flex-1 h-12 rounded-2xl bg-mint text-black font-black text-sm disabled:opacity-50">
                 قيّم إجابتي
                </button>
                <button
                  onClick={() => {
                  ...
                  className="h-12 px-4 border border-white/20 rounded-2xl"
                >
                  🎤
                </button>
              </div>
              ...
              {evalRes && (
                <div className="mt-4 p-4 bg-black/30 border border-mint/30 rounded-2xl text-sm">
                  <span className="text-4xl font-black text-mint">{evalRes.percentage}%</span>
                  <div className="mt-1 text-white/80">{evalRes.feedback}</div>
                  <button
                    onClick={() => {
                    ...
                    className="mt-2 text-xs border border-mint/30 rounded px-3 py-1 text-mint"
                  >
                    استمع للنموذج
                  </button>
                </div>
  ...
  return (
    <div dir="rtl" className="pb-12">
      <div className="mb-5 rounded-3xl bg-gradient-to-br from-emerald-700 to-green-800 px-6 py-6 text-white">
        <div className="text-xs opacity-80">GEN Z • تفاعلي • إطار بإطار</div>
        <h1 className="text-3xl font-black tracking-[-1px] mt-1">{lessonTitleAr}</h1>
        <div className="text-sm opacity-75 mt-1">اسحب أفقياً إطار بإطار</div>
      </div>
      ...
      <div className="relative">
        ...
        <div
          ...
          className="flex overflow-x-auto snap-x snap-mandatory pb-4 -mx-1 px-1"
      ...
      <div className="mt-8">
        <div className="flex items-center gap-3 mb-3 px-1">
          ...
          <div className="font-black text-white">تقويم الدرس</div>
        </div>
        ...
        {!qcmDone ? (
          ...
          <div className="rounded-3xl bg-white/5 p-5 border border-white/10">
            {[
              { q: "ما دور Ca²⁺؟", o: ["إفراز ACh", "إغلاق القنوات"], c: 0 },
              { q: "تأثير Curare؟", o: ["شلل", "زيادة الإشارة"], c: 0 },
            ].map((item, idx) => (
