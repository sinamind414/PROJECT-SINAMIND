...
export default function GenZHeroMission({
  title = "Volcans et leur distribution",
  subtitle = "Carte + distribution + analyse",
  duration = "12 min",
  points = 7,
  onStart,
  ...
  const handleStart = () => {
    if (onStart) onStart()
    else router.push("/lecons-sciences-experimentales")
  }

  return (
    <div className="mx-4 mt-2 rounded-3xl bg-gradient-to-br from-emerald-500/10 via-mint/10 to-slate-900/60 border border-mint/30 p-5 sm:p-6">
      ...
      <div className="flex items-center gap-3 mt-4 text-sm">
        ...
        <div className="font-bold text-emerald-400">+{points} points</div>
      </div>
      ...
      <p className="text-center text-[10px] text-white/50 mt-2.5">
        Tu peux gagner ces points en moins de 15 minutes
      </p>
    </div>
  )
}
