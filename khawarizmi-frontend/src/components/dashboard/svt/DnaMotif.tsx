export function DnaMotif({ className = "", size = 120 }: { className?: string; size?: number }) {
  return (
    <svg width={size} height={size} viewBox="0 0 120 120" className={`${className} svt-dna-spin`} fill="none" style={{ transformOrigin: "60px 60px" }}>
      {Array.from({ length: 11 }).map((_, i) => {
        const t = i / 10; const y = 10 + t * 100; const phase = t * Math.PI * 2
        const x1 = 60 + Math.sin(phase) * 34; const x2 = 60 - Math.sin(phase) * 34
        return (
          <g key={i}>
