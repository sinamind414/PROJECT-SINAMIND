...
import { diagnosticScenario, type DiagnosticQuestion } from "@/lib/methodology-documents"
...
type QuestionId = DiagnosticQuestion["id"]
...
function buildPriorityFixes(evaluations: DiagnosticResult["evaluations"]) {
  ...
  evaluations.forEach((item) => {
    ...
    if (item.question.id === "analyse") fixes.push("تدرّب على تحليل وثيقة دون استعمال لأن/بسبب، مع ذكر القيم العددية.")
    if (item.question.id === "interpret") fixes.push("اربط كل تفسير بملاحظة ثم بسبب علمي واضح من الوثيقة والدرس.")
    if (item.question.id === "deduce") fixes.push("اكتب الاستنتاج في جملة واحدة تبدأ بـ نستنتج أن.")
    if (item.question.id === "hypothesis") fixes.push("صغ الفرضية كسبب محتمل مرتبط بالوثيقة، وليس كتخمين عام.")
    if (item.question.id === "scientific-text") fixes.push("ابنِ النص العلمي وفق: مقدمة + إشكالية؟ + عرض يستغل الوثائق + خاتمة.")
  })
...
function buildDiagnosticResult(answers: Record<string, string>): DiagnosticResult {
  const evaluations = QUESTIONS.map((question) => ({
    ...
    evaluation: evaluateMethodologyAnswer({ verbSlug: question.id, answer: answers[question.id] || "" }),
  }))
