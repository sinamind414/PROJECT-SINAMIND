...
echo "📦 Installing shadcn/ui Radix dependencies..."
...
# Init shadcn/ui (non-interactive)
echo "⚙️  Initializing shadcn/ui..."
npx shadcn-ui@latest init -y --defaults 2>/dev/null || true

# Add all components via shadcn CLI
echo "📦 Adding shadcn/ui components via CLI..."
SHADCN_COMPONENTS="accordion alert aspect-ratio avatar badge breadcrumb button calendar card carousel checkbox collapsible command context-menu dialog drawer dropdown-menu form hover-card input label menubar navigation-menu popover progress radio-group resizable scroll-area select separator sheet skeleton slider sonner switch table tabs textarea toast toggle toggle-group tooltip"
for component in $SHADCN_COMPONENTS; do
  echo "  → Adding $component..."
  npx shadcn-ui@latest add "$component" -y 2>/dev/null || true
done
